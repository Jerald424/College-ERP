...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/transport/bus-session/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/transport/bus-session" });
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Bus delete successfully");
        },
        onError: (error) => {
