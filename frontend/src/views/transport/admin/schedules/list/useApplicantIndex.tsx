...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/transport/bus-schedule/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/transport/bus-schedule" });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        bus: `${res?.session?.bus?.name}-${res?.session?.name}`,
        type: res?.type?.value,
        status: res?.is_entered ? <Tag color="green">Marked</Tag> : <Tag color="orange">Not Marked</Tag>,
        actions: (
          <div>
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Bus schedule delete successfully");
        },
        onError: (error) => {
