...
export default function DepartmentCreateEdit() {
  ...
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Department", path: "/divider/department" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update department" : "Create department"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update department" : "Create department"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
