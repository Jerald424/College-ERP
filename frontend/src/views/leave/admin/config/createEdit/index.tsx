...
import AllocateLeave from "./allocateLeave";

export default function StaffLeaveConfigCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Leave Type", path: "/divider/leave/leave-config" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update leave type" : "Create leave type"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update leave type" : "Create leave type"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
        ...
        <>
          ...
          <AllocateLeave leave_type_id={record_id} />
        </>
      )}
    </div>
  );
