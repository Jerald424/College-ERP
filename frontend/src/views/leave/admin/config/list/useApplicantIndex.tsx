...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/leave/config/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/leave/config" });
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Leave type delete successfully");
        },
        onError: (error) => {
