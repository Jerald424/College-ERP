...
const deleteApplicant = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/student/student/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: `api/student/student`, is_academic_year_based: true, query: "&status=in_progress" });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        name: (
          <div>
            <img src={res?.applicant?.image} style={{ height: "50px", width: "50px", borderRadius: 50 }} className="me-3" />
            {res?.applicant?.name}
          </div>
        ),
        roll_no: res?.roll_no,
        programme: res?.applicant?.programme?.name,
        actions: (
          <div>
