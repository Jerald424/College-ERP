...
const deleteApplicant = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/admission/applicants/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/admission/applicants", is_academic_year_based: true });
  ...
  const handleEditOrCreate = (arg?: any) => {
    let str = `/divider/admission/applicant/${arg?.id}`;
    navigate(str);
  };
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        name: (
          <div>
            <img className="me-3" src={res?.image} style={{ height: 50, width: 50, objectFit: "cover", borderRadius: 50 }} />
            {res?.name}
          </div>
        ),
        programme: res?.programme?.name,
        status: <Tag color={status_color?.[res?.status?.id]}>{res?.status?.value}</Tag>,
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Applicant delete successfully");
        },
        onError: (error) => {
