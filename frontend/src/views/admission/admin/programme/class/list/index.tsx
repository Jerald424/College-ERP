...
export default function ProgrammeClass() {
  const { isLoading, columns, dataSource, pagination, setPagination, total_count, Modal, selected, setIsOpen, handleDelete, programme } = useApplicant();

  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Programme", path: "/divider/admission/programme" }, { title: programme?.name, path: `detail?id=${programme?.id}` }, { title: "Class" }]} />

      <CardCmp
        title={
          <div>
            <span className="me-3">Class</span>
            <Link to={`/divider/admission/class/${programme?.id}/detail`}>
