...
const deleteApplicant = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/admission/programme/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/admission/get-programme" });
  ...
  const columns = [
    ...
    {
      dataIndex: "programme_programme_level",
      title: "Level",
    },
    {
      dataIndex: "actions",
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        programme_programme_level: res?.programme_programme_level?.value,
        actions: (
          <div>
