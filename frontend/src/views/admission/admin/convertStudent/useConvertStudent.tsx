...
const getApplicant = async ({ programme_id, academic_year_id }: { programme_id: number; academic_year_id?: number }) => {
  ...
  const response = await axiosInstance.get(`api/admission/selected-applicant/${programme_id}?attributes=id,name,gender,image,application_no&academic_year_id=${academic_year_id}&status=selected`);
  return response?.response;
};

export default function useConvertStudent() {
  ...
  const handleSelect = () => {
    try {
      ...
      selectApplicantMutate(
        { applicant_ids: selected, status: "student_created" },
        {
          onSuccess: () => {
