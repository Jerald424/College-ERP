...
export default function AdminCreateEditApplicantFee() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Applicant Fee", path: "/divider/admission/applicant-fee" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update application fee" : "Create application fee"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update applicant fee" : "Create applicant fee"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
