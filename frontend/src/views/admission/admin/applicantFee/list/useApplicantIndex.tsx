...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/admission/application-fees", is_academic_year_based: true });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        name: res?.name,
        programme_level_id: res?.programme_level_id?.value,
        amount: res?.amount,
        actions: (
          <div>
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Applicant fee delete successfully");
        },
        onError: (error) => {
