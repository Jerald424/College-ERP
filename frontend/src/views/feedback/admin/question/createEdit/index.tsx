...
export default function FeedbackQuestionCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit, data, watch, reset, type } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Feedback Question", path: "/divider/feedback/question" }, { title: record_id ? "Update" : "Create" }]} />
      <LoaderWithChildren isLoading={isLoading}>
        <form onSubmit={handleSubmit}>
          <CardCmp title={<Header isEdit={isEdit} setIsEdit={setIsEdit} record_id={record_id} />} className="mt-2">
            ...
            {record_id && ["dropdown-single", "dropdown-multiple"].includes(type) && <QuestionOptions isEdit={isEdit} reset={reset} watch={watch} />}
          </CardCmp>
        </form>
      </LoaderWithChildren>
      <Modal description={record_id ? "Update course" : "Create course"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update course" : "Create course"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
