...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/feedback/form/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/feedback/form" });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        date: (
          <div>
            {makeJsDateToDDMMYYYY(res?.start_date)} {getHHMMFromJsDate(res?.start_date)}
            <ArrowRightOutlined className="mx-3" />
            {makeJsDateToDDMMYYYY(res?.end_date)} {getHHMMFromJsDate(res?.end_date)}
          </div>
        ),
        type: <Tag color="blue">{res?.feedback_for?.value}</Tag>,
        actions: (
          <div>
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Questions delete successfully");
        },
        onError: (error) => {
