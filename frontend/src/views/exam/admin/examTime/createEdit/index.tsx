...
export default function ExamTimeCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Exam time", path: "/divider/exam/exam-time" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update Exam time" : "Create Exam time"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update exam time" : "Create exam time"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
