...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/exam/exam-timetable/${ids}`);
  return response;
};

export default function useApplicant() {
  const [search] = useSearchParams();
  let exam_id = search.get("exam_id");
  const {
    data: { data, refetch, isPending, isFetching },
    paginate: { pagination, total_count, setPagination },
  } = useFetchPaginatedData({ url: "api/exam/schedules", query: exam_id ? `&exam_id=${exam_id}` : "" });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        class: res?.class?.acronym,
        date: res?.date,
        time: res?.exam_time?.name,
        exam: res?.exam?.name,
        room: res?.exam_room?.name,
  ...
  const handleDelete = () => {
    ...
    userDeleteMutate(
      ...
      {
        onSuccess: () => {
          message.success("Domain url delete successfully");
        },
        onError: (error) => {
  ...
  return { isLoading: isPending || isLoadingRoleDelete, columns, dataSource, pagination, setPagination, total_count, Modal, selected, setIsOpen, handleDelete, isFetching };
}
