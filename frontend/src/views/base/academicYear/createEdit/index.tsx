...
import useCreate, { getTermCount } from "./useCreate";
import { Link } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";

export default function AcademicYearCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Academic Year", path: "/divider/base/academic-year" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update academic year" : "Create academic year"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  const { data } = useQuery({ queryKey: ["get/term-count"], queryFn: () => getTermCount({ academic_year_id: record_id }) });

  return (
    <div className="df">
      <div className="f1">
        {record_id ? "Update academic year" : "Create academic year"}
        {isEdit || !record_id ? (
          <>
            {record_id && (
      ...
      <Link to={`/divider/base/academic-year/term?academic_year_id=${record_id}`}>Term [{data?.count}]</Link>
    </div>
  );
};
