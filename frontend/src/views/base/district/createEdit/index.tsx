...
export default function DistrictCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "District", path: "/divider/base/district" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update district" : "Create district"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update district" : "Create district"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
