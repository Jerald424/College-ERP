...
export default function DomainUrlCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, navigate, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Domain url", path: "/divider/base/domain-url" }, { title: record_id ? "Update" : "Create" }]} />
      ...
      <Modal description={record_id ? "Update Domain url" : "Create Domain url"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update Domain url" : "Create Domain url"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
