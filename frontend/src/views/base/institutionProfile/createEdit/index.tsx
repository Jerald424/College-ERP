...
export default function InstitutionProfileCreateEdit() {
  const { control, form_data, record_id, Modal, handleSubmit, description, isLoading, onSubmit, isEdit, setIsEdit } = useCreate();
  return (
    <div>
      <Breadcrumb className="ms-3 mb-2" items={[{ title: "Institution Profile", path: "/divider/base/institution-profile" }, { title: record_id ? "Update" : "Create" }]} />
      <LoaderWithChildren isLoading={isLoading}>
        <form onSubmit={handleSubmit}>
          <CardCmp title={<Header isEdit={isEdit} setIsEdit={setIsEdit} record_id={record_id} />} className="mt-2">
            ...
            <FormWithHook is_edit={isEdit || !record_id} control={control} data={description} />
          </CardCmp>
        </form>
      </LoaderWithChildren>
      <Modal description={record_id ? "Update institution profile" : "Create institution profile"} okButtonProps={{ onClick: onSubmit }} />
    </div>
  );
}
...
const Header = ({ record_id, isEdit, setIsEdit }: { record_id: number; isEdit: boolean; setIsEdit: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div>
      {record_id ? "Update institution profile" : "Create institution profile"}
      {isEdit || !record_id ? (
        <>
          {record_id && (
