...
const deleteRecord = async ({ ids }: { ids: string }) => {
  const response = await axiosInstance.delete(`api/institution/institution-profile/${ids}`);
  return response;
};
...
export default function useApplicant() {
  ...
  } = useFetchPaginatedData({ url: "api/institution/institution-profile" });
  ...
  let dataSource = useMemo(() => {
    try {
      return data?.response?.rows?.map((res, index) => ({
        ...
        name: (
          <>
            <img src={res?.image} style={{ height: 50, width: 50, borderRadius: 50, objectFit: "cover" }} className="me-3" />
            {res?.name}
          </>
        ),
        academic_year: res?.academic_year?.name,
        active: <SwitchCmp checked={res?.is_active} />,
        actions: (
          <div>
